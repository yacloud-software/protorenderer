...
func (j *JavaCompiler) Compile() error {
	...
	if !*java_use_std_protoc {
		...
		compiler_exe, err = utils.FindFile("extra/compilers/weird.exe")
		if err != nil {
	...
	plugin_exe, err := utils.FindFile(fmt.Sprintf("extra/compilers/%s", *java_plugin_name))
	if err != nil {
...
func (j *JavaCompiler) CompileToClass() error {
	...
	jars, err := utils.FindFile("extra/jars")
	if err != nil {
